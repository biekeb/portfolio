...
export default function Contact() {
    ...
    return (
        <div id="contact" className="container max-w-[1320px] mx-auto px-5 md:px-10 xl:px-5 pt-24 xl:pt-28">
            <motion.div 
                className="w-full lg:flex space-y-6 lg:space-y-0"
            ...
            >
                ...
                {/* Right Form Section */}
                <motion.div
                    className="w-full lg:w-2/3 mt-8 lg:text-right"
                    initial={{ opacity: 0, x: 50 }}
                    whileInView={{ opacity: 1, x: 0 }}
                    viewport={{ once: true }}
                ...
                >
                    ...
                    <div className="submit-result mt-4">
                        {status === 'success' && (
                            <span className="transition duration-200 ease-out text-green-700">Thank you! Your Message has been sent.</span>
                        )}
                        {status === 'error' && (
                            <span className="transition duration-200 ease-out text-red-600">Something went wrong. Please try again!</span>
                        )}
                    </div>
                </motion.div>
            </motion.div>
